...
4. Read `data/PLAN.md`.
5. Read `data/progress.json`.
6. Read `data/RESUME.md`.
7. Read `data/validation-report.json`.
...
2. inspect `progress.json`;
3. identify the most important coverage gaps;
4. define the batch priorities from those gaps;
5. begin at `progress.json.candidateAcquisition.nextCandidateSequence` and process `data/recipe_name_candidates.json` strictly in order;
...
For recipe images, prefer an exact source-page photograph, then a distinct related real-food photograph, and use AI only as the final fallback after both real-image routes fail. Related images may be approximate but must visibly fit the dish; photographer watermarks are allowed. For ingredients, prefer the exact Walmart product-page image. Preserve honest provenance and unverified-rights status, keep unresolved images explicitly pending, and run `npm run images:queue` (or `npm run build`) so both `research/image` files remain complete handoff lists.
...
7. update `validation-report.json`;
