...
This directory contains an optional secondary image tool that searches Wikimedia
Commons for **real, non-AI photographs** and converts accepted photos into
Appetee AVIF assets.

It is an optional staging workflow and is not invoked by the normal npm build, validation, or checkpoint commands. Review its output before mapping any accepted asset and provenance into a canonical recipe record.
...
master `data/candidates/recipe-names.json`, and map any accepted image through the
...
python tools\\acquire_recipe_images.py --input ..\\..\\..\\candidates\\recipe-names.json --output ..\\..\\..\\research\\image\\wikimedia
```
