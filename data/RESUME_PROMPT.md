...
If validation passes, resume from `nextRecipeSequence` and `candidateAcquisition.nextCandidateSequence` in `data/progress.json`.
...
- process `data/recipe_name_candidates.json` strictly in sequence and document distribution drift;
...
Set both `mealType` and `mealCategory` on every recipe. For recipe images, try the exact source page first, then a distinct related real-food photo; use AI only as the last fallback after both fail. Prefer the exact Walmart product-page image for ingredients. Mark unresolved images pending and regenerate both `research/image` handoff queues.
...
Run `npm run images:source` for exact source/product images and `npm run images:related` for still-pending recipes. Visually audit related results. Run `npm run images:queue` or `npm run build` so the owner receives complete recipe-name/source-URL and ingredient-name/Walmart-URL lists for anything still pending.
